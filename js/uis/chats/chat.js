export default class ChatItem extends HTMLDivElement {
  ...
  getUserInfo = () => {
    ...
    const company = this.getAttribute('user-company');
    if (profession && company) return /* html */`${profession} <span class="sp">•</span> ${company}`;
    return /* html */`Member <span class="sp">•</span>${company || 'Thealcohesion'}`;
  }
    ...
    const innerAttachements = attachements.map(attachement => {
      return /* html */`
        <a href="${attachement.link}" target="_blank" download
          ...
          <span class="size">${attachement.size}</span>
          ${attachement.name}
        </a>
      `;
    }).join('');
