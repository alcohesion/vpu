export default class VerifyEmail extends HTMLElement {
  constructor() {
    ...
    form.addEventListener('submit', async e => {
      ...
      if (result.validated && result.values) {
        ...
        if(data.verified) {
          ...
          this.app.navigate({ kind: 'verification', no: 3, name: data.user.name });
        }
      } else {
  ...
  generateCode = async () => {
    const generateButton = this.querySelector('.email-header > .actions > .action');
    const form = this.querySelector('#name-form');
    ...
    generateButton.addEventListener('click', async e => {
      ...
      try {
        ...
        // add success message
        this.addSeverMessage(form, 'Code sent successfully, check your mailbox', 'success');
      } catch (error) {
        // add server message
        this.addSeverMessage(form, error.message);
  ...
  getHeader = () => {
    return /* html */`
      <div class="email-header">
        <div class="top">
          <h4 class="title">Verify Email Address</h4>
          <p class="desc">
            Please click the send code button below to request a verification code to <b>${this.getAttribute('email')}</b>.
            You will receive a verification code via your email address.
          </p>
        </div>
        <div class="actions">
          <button class="action" id="verify-email">Send code</button>
        </div>
      </div>
    `;
  }
    ...
    return /* css */`
      <style>
        ...
        *::-webkit-scrollbar {
        ...
        .email-header > .actions > .action.sent {
