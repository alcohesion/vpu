export default class IdentityPicture extends HTMLElement {
  constructor() {
    super();
  ...
  handleBack = () => {
    // get back button
    const backButton = this.querySelector('.buttons > .button.prev');
  ...
  submitForm = form => {
    form.addEventListener('submit', async e => {
      ...
      if (upload.uploaded) {
        // set values to app
        this.app.setIdentityData({ picture: upload.url });

        // navigate to next form
        this.app.navigate({ kind: 'identity', no: 2 });
      } else {
  ...
  getBody() {
    return /*html*/`
      <form class="fields picture">
        ...
        <input type="hidden" id="captured-image" name="captured-image" value="">
        ${this.getButtons()}
      </form>
    `;
  }
    ...
    return /* html */`
      <div class="content">
        <div class="top">
          ...
          <p class="desc">
            Click <strong><i>Take Photo</i></strong> to capture a picture of yourself.
            <br>You can <strong><i>Retake Photo</i></strong> if it is not looking good.
          </p>
        </div>
      </div>
    `;
